...
- **Hermes is a `Plugin`.** Master ships `NullPlugin` from `src/mu/coding/active.mojo`. The hermes branch overwrites that one file and adds `src/mu/hermes/`. Shared CLI, loop, and config stay identical.
